...
| FAS workflow policy ownership | `Partial` | [../../fas/docs/adr/0006 - shared-architecture-roadmap.md](../../fas/docs/adr/0006%20-%20shared-architecture-roadmap.md) is proposed and actor-web boundary notes reinforce the direction, but actor-web cannot ratify FAS policy ownership | FAS continues to own policy even if runtime mechanics move out |
| fas-local public runtime semantics | `Partial` | [./0009-fas-local-runtime-host-substrate-alignment.md](./0009-fas-local-runtime-host-substrate-alignment.md) is proposed boundary guidance that keeps `Runtime`, `Session`, `Provider`, and CLI meaning in `fas-local` | actor-web may host internals, but not own public semantics |
