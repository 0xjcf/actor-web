...
- packages/actor-core-runtime/src/unit/browser-entrypoint.test.ts

## Scope Amendments

- Type: implementation-scope
- Added at: 2026-05-29T04:21:00Z
- Trigger: Closeout readiness reported runtime-gateway-shared.ts as an unexpected source file after implementation.
- Reason: The architect and staff-engineer handoffs selected a new browser-safe shared gateway contracts/source-handle module as the minimal no-dependency fix; browser-entrypoint.test.ts is the focused regression that verifies the browser source graph does not reach node:crypto or crypto.
- Added paths: packages/actor-core-runtime/src/runtime-gateway-shared.ts, packages/actor-core-runtime/src/unit/browser-entrypoint.test.ts
- Evidence source: delegated handoffs and closeout-readiness
- Evidence: delegated handoffs and closeout-readiness | .fas/state/closeout-readiness/latest.json | runtime-gateway-shared.ts was the sole unplanned source file; package.json and docs remained reference-only because no dependency/export/doc wording change was needed.
- Accuracy signal: Focused tests and typecheck passed; fas validate-task fast checks passed before scope gate.
- Follow-up needed: Consider a future dist/package-export browser smoke before publish if release packaging confidence needs to increase.
