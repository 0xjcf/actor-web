...
import { createActorRef, createRootActor, XStateActorRefAdapter } from './xstate-adapter';
import { 
  testMachines, 
  counterMachine, 
  trafficLightMachine, 
...
describe('XStateActorRefAdapter', () => {
  ...
  describe('Basic ActorRef Compliance', () => {
    it('should implement ActorRef interface', () => {
      actorRef = createActorRef(counterMachine);
    ...
    it('should generate unique IDs when not provided', () => {
      ...
      expect(actor1.id).toMatch(/^actor-\d+-\w+$/);
    });

    it('should use custom ID when provided', () => {
      actorRef = createActorRef(counterMachine, { id: 'my-counter' });
  ...
  describe('Traffic Light Machine Tests', () => {
    beforeEach(() => {
      actorRef = createActorRef(trafficLightMachine);
      actorRef.start();
    });
  ...
  describe('Delayed Machine Tests', () => {
    it('should handle delayed transitions', async () => {
      actorRef = createActorRef(delayedMachine);
      actorRef.start();
  ...
  describe('Error Prone Machine Tests', () => {
    beforeEach(() => {
      actorRef = createActorRef(errorProneMachine);
      actorRef.start();
    });
  ...
  describe('Guarded Machine Tests', () => {
    beforeEach(() => {
      actorRef = createActorRef(guardedMachine);
      actorRef.start();
    });
  ...
  describe('Ask Pattern Tests', () => {
    it('should handle ask queries', async () => {
      actorRef = createActorRef(queryMachine);
      actorRef.start();
      ...
      // Current implementation returns query as response
      const query = { type: 'QUERY', key: 'name', correlationId: '123' };
      const response = await actorRef.ask(query);
    ...
    it('should reject ask on stopped actor', async () => {
      ...
      await expect(actorRef.ask({ type: 'QUERY', key: 'test' })).rejects.toThrow(
        'Cannot query stopped actor'
      );
    });
  });
  ...
  describe('Child Actor Management', () => {
    ...
    it('should stop all children when parent stops', async () => {
      ...
      children.forEach(child => expect(child.status).toBe('active'));
  ...
  describe('Supervision Tests', () => {
    it('should create supervisor for actors with supervision strategy', () => {
      ...
      const adapter = actorRef as XStateActorRefAdapter;
      expect(adapter['supervisor']).toBeDefined();
    });
    ...
    it('should escalate errors to parent', () => {
      ...
      const child = createActorRef(errorProneMachine, {
        parent,
        supervision: 'escalate'
      });
      
      const adapter = child as XStateActorRefAdapter;
      adapter['handleError'](new Error('Child error'));
  ...
  describe('Factory Functions', () => {
    it('should create root actor with supervision', () => {
      const rootActor = createRootActor(counterMachine, { id: 'root-counter' });
    ...
    it('should pass through options to root actor', () => {
      const onMessage = vi.fn();
      const rootActor = createRootActor(counterMachine, {
        askTimeout: 2000,
        metrics: { onMessage }
      });
  ...
  describe('Complex Machine Integration', () => {
    it('should work with complex state machines', async () => {
      ...
      actorRef = createActorRef(queryMachine, { id: 'query-actor' });
      actorRef.start();
