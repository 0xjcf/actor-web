  ...
  it('packs a clean consumer-ready artifact with rewritten workspace dependencies', async () => {
    ...
    await writeFile(
      ...
      [
        ...
        '',
        'const traces = [];',
        'async function waitForTraceStrings(expected, timeoutMs = 1500) {',
        '  const deadline = Date.now() + timeoutMs;',
        '  while (Date.now() < deadline) {',
        '    const traceJson = JSON.stringify(traces);',
        '    if (expected.every((entry) => traceJson.includes(entry))) {',
        '      return traceJson;',
        '    }',
        ...
        'const interrupted = await remote.ask("controlPlaneSession", JSON.stringify({ type: "INTERRUPT_AGENT_SESSION", sessionId: "resume-session-1" }));',
        ...
        'const resumed = await remote.ask("controlPlaneSession", JSON.stringify({ type: "RESUME_AGENT_SESSION", sessionId: "resume-session-1" }));',
        'if (!resumed.ok) throw new Error(resumed.error);',
        'await remote.flush();',
        '',
        'traceWatch.value();',
        'const traceJson = await waitForTraceStrings(["interruption_resume", "resume_requires_reconciliation"]);',
        'if (!traceJson.includes("interruption_resume")) throw new Error("trace watch missed interruption_resume");',
        'if (!traceJson.includes("resume_requires_reconciliation")) throw new Error("trace watch missed resume reconciliation detail");',
        '',
        'await remote.stop();',
        'await server.stop();',
