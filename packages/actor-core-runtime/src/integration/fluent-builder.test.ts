...
interface SetValueMessage extends ActorMessage {
  type: 'SET_VALUE';
  value: number;
}

interface TriggerErrorMessage extends ActorMessage {
  type: 'TRIGGER_ERROR';
}

interface BatchProcessMessage extends ActorMessage {
  type: 'BATCH_PROCESS';
  items: number[];
}
...
describe.skip('Fluent Builder API Integration', () => {
  ...
  describe.skip('Context-Based Actor Creation', () => {
          ...
          try {
            if (message.type === 'SET_VALUE') {
              ...
              if (typeof value !== 'number') {
                log.warn('Invalid payload type received', { message });
                return {
  ...
  describe.skip('Actor Lifecycle and Supervision', () => {
    ...
    it('should recover from errors gracefully', async () => {
      ...
      // Trigger error
      const errorResponse = await actorRef.ask<CounterContext>({
        type: 'TRIGGER_ERROR',
      });
      expect(errorResponse.status).toBe('error');

      // Test recovery
      const recoveryResponse = await actorRef.ask<CounterContext>({
        type: 'INCREMENT',
      });
  ...
  describe.skip('OTP Integration Debugging', () => {
